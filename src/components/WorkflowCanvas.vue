...
const { project, onConnect, toObject, setViewport, onViewportChange, fitView } =
 useVueFlow();
...
// Optimize edges: only animate if graph is not too large (performance optimization)
const optimizedEdges = computed(() => {
 const edgeCount = edges.value.length;
 const shouldAnimate = edgeCount < 100; // Disable animations for large graphs
  return edges.value.map(edge => ({
   ...edge,
   animated: shouldAnimate && edge.animated,
 }));
});


// --- AUTO-CLOSE PREVIEW BOX AFTER 1.5 SECONDS ---
let logsClearTimeout: ReturnType<typeof setTimeout> | null = null;


const clearLogsAfterDelay = () => {
 // Clear any existing timeout
 if (logsClearTimeout) {
   clearTimeout(logsClearTimeout);
 }
 // Set new timeout to clear logs after 1.5 seconds
 logsClearTimeout = setTimeout(() => {
   store.logs = [];
 }, 1500);
};


// Watch for log changes and reset the timeout
watch(
 () => store.logs.length,
 () => {
...
// Watch only the nodes/edges arrays, not deep properties
watch(
 () => [store.present.nodes.length, store.present.edges.length],
 () => debouncedSave(),
);


// --- 4. DEBOUNCED NODE DRAG FOR HISTORY ---
const debouncedNodeDragStop = useDebounceFn(() => {
 if (store.pendingNodeDrag) {
   const snapshot = toObject();
   store.commit({ nodes: snapshot.nodes, edges: snapshot.edges }, 'Move Node');
   store.pendingNodeDrag = false;
 }
}, 300);


// --- 5. AUTO-PAN TO ACTIVE NODE DURING PREVIEW ---
const panToActiveNode = async () => {
 if (!store.activeNodeId || !store.isRunning) return;
  // Find the active node
 const activeNode = store.present.nodes.find(n => n.id === store.activeNodeId);
 if (!activeNode) return;
  // Wait for next tick to ensure the node is rendered
 await nextTick();
 ...
 const nodeHeight = 50;
 const zoom = 1.2;
  // Calculate the node center in canvas coordinates
 const nodeCenterX = activeNode.position.x + nodeWidth / 2;
 const nodeCenterY = activeNode.position.y + nodeHeight / 2;
  // Calculate viewport offset to center node on screen
 // Formula: screenCenter = (canvasPoint * zoom + viewportOffset)
 // Therefore: viewportOffset = screenCenter - (canvasPoint * zoom)
 const x = window.innerWidth / 2 - nodeCenterX * zoom;
 const y = window.innerHeight / 2 - nodeCenterY * zoom;
  setViewport({ x, y, zoom }, { duration: 400 });
};


// Watch active node during preview to pan viewport
watch(
 () => store.activeNodeId,
 () => {
   if (store.isRunning) {
...
const isValidConnection = (connection: any) => {
 ...
 // Allow all other connections - validation happens in onConnect
 return true;
};
...
function onDrop(event: DragEvent) {
 ...
 recordHistory(
   { ...store.present, nodes: [...store.present.nodes, newNode] },
   `Add ${type} Node`,
 );
}
...
onConnect((params) => {
 // Validate cycle and type mismatch only when actually connecting
 ...
 if (sourceType && targetType && sourceType !== targetType) {
   store.addLog(
     "system",
     "error",
     `Type mismatch: ${sourceType} vs ${targetType}`,
   );
   return;
 }
 ...
 recordHistory(
   { ...store.present, edges: [...store.present.edges, newEdge] },
   "Connect Nodes",
 );
});
...
function onEdgeUpdate({ edge, connection }: any) {
 ...
 recordHistory(
   { nodes: store.present.nodes, edges: [...otherEdges, updatedEdge] },
   "Update Connection",
 );
}
</script>
...
<template>
 ...
 >
   <VueFlow
     ...
     @node-drag-stop="() => { store.pendingNodeDrag = true; debouncedNodeDragStop(); }"
   ...
   >
     ...
     <div
       v-if="store.validationWarnings.length"
       class="absolute top-20 left-4 z-50 bg-yellow-50 border-l-4 border-yellow-400 p-4 text-yellow-800 rounded shadow-lg max-w-sm"
     >
       <div
         v-for="(warning, i) in store.validationWarnings"
         :key="i"
       ...
       <div
         v-if="store.startNodes.length > 1"
         class="mt-3 pt-3 border-t border-yellow-300"
       >
         <p class="text-xs font-semibold mb-2">
           Select which start node to execute:
         </p>
         <div class="flex flex-col gap-2">
           <button
             v-for="node in store.startNodes"
             :key="node.id"
             @click="store.setSelectedStartNode(node.id)"
             :class="[
               'px-3 py-1 text-xs rounded font-semibold transition-colors text-left',
               store.selectedStartNodeId === node.id
     ...
     <div
       class="absolute bottom-6 left-6 z-50 flex items-center gap-4 bg-white p-2 rounded-lg shadow-lg border border-slate-200"
     >
       <div class="flex gap-2">
         <button
           @click="store.runSimulation"
           :disabled="store.isRunning"
           class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded font-bold disabled:opacity-50 transition-colors"
         >
           {{ store.isRunning ? "Running..." : "▶ Run Preview" }}
         </button>
         <button
           v-if="store.isRunning"
           @click="store.isRunning = false"
           class="bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded font-bold"
         >
           ⏹ Stop
         </button>
       </div>
   ...
   <div
     ...
     class="absolute bottom-24 left-6 w-72 max-h-48 overflow-y-auto bg-slate-900/90 text-white p-3 text-[11px] rounded-lg shadow-2xl z-50 font-mono border border-slate-700"
   >
     <div
       v-for="(log, i) in store.logs"
       :key="i"
       class="mb-1 border-b border-slate-800 pb-1 last:border-0"
     >
       ...
       <span
         :class="{
           ...
           'text-rose-400': log.status === 'error',
         }"
         class="ml-2"
       >
         {{ log.message }}
